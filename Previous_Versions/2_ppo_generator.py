...
def get_relevant_articles(topic, article_dict, openai_api_key):
...
def recheck_for_client(client, article_dict, openai_api_key):
    chat_gpt_error = False
    chat_gpt_error_str = ''
...
def client_media_opportunities(client, article_dict, opportunity_name, openai_api_key):
    chat_gpt_error = False
    chat_gpt_error_str = ''
    ...
    if article_dict:
        ...
        for article_dict_keys in article_dict_keys_list:
            article_id = article_dict_keys
            article = article_dict[article_dict_keys]
            article_prompt += f"```Article id {article_id} and article text is {article} \n ```"
        try:
            URL = "https://api.openai.com/v1/chat/completions"
            ...
            Tata Power is a power company with focus on energy saving aspect of sustainability
            Your task is to help a communications team of {client} build a content using news jacketing exercise

            You have to first select best suited story for this exercise from the articles provided in News Articles delimited by triple backticks.

            For the selected article generate a media opportunity / idea of {opportunity_name} in not more then 8 lines
            ...
            Can you please provide output in JSON like:-\n \
            {{
            ...
            if response.status_code == 200:
                ...
                ChatGptwriteLog(str("ChatGpt for media opportunities articles Input for Article ID {} is:- \n {}\n").format(str(article_id), prompt))
                ChatGptwriteLog(str("ChatGpt for media opportunities articles Response for Article ID {} is:- \n {}\n").format(str(article_id), output_response))
                ...
                if ('id' in output_json) and ('media opportunity' in output_json):
    ...
    return best_match_found, best_article_id, media_opportunities
...
def main():
    ...
    ### Order by ArticleID 
    # cursor.execute('''SELECT * [ArticleID],
    #         a.[EntityID],
    #         EntityName,
    #         [Url],
    #         [Date],
    ...
    #         isnull([Text],'') as Text,
    #         Isnull([Title],'') +' '+isnull([Text],'') as TitleText
    #     FROM ['''+database+'''].[dbo].[Article] a with (nolock)
    #     inner join Entity e on e.EntityID=a.EntityID
    #     where IsSencheckProcessed=0''')
    ...
    for row in topic_list:
        ...
        try:
            if topic_name:
                ...
                if article_dict:
                    relevant_article_list = get_relevant_articles(topic_name, article_dict, openai_api_key)
                    ...
                    if relevant_article_list:
                        ...
                        for row in client_list:
                            ...
                            article_for_client_list = recheck_for_client(client_name, relevant_article_dict, openai_api_key)
                            ...
                            if article_for_client_list:
                                ...
                                if len(opportunity_list) > 0:
                                    ...
                                    for row in opportunity_list:
                                        ...
                                        best_match_found, best_article_id, media_opportunities = client_media_opportunities(client_name, article_dict, opportunity_name, openai_api_key)

                                        if best_match_found:
                                            answer_insert_query = "exec [USP_AnswerPPO_Insert] @APID=?, @TopicID=?, @EOID=?, @ClientID=?, @ArticleId=?, @Opportunity=?"
                                            answer_values = (int(ap_id), int(topic_id), int(opportunity_id), int(client_id), int(best_article_id), str(media_opportunities))
