...
def get_similar_docs_id(vectorstore, topic_name, sub_topic):
    metadata_field_info=[
        AttributeInfo(
            name="id",
    ...
    if sub_topic:
        input_text = "Top 5 Articles which are about {}".format(sub_topic)
    else:
    ...
    log_str = f"Retriever query for Pinecone is:- {input_text}"
    print(log_str)
    writeLog(log_str)
...
def get_best_id_source_chain(topic_id, topic_name, sub_topic, client_name, client_article_details_dict, emailids):
    ...
    #green financing
    if sub_topic:
        query = f"""What is most relevant text about {sub_topic} suitable for news jacketing exercise?
                    If there is no article suitable for the exercise of news jacketting then simply respond \"No best match.\"
                """
    else:
        query = f"""What is most relevant text about {topic_name} suitable for news jacketing exercise?
                    If there is no article suitable for the exercise of news jacketting then simply respond \"No best match.\"
                """
    ...
    if article_dict:
        ...
        while loop_continue:
            try:
                ...
                question_prompt = f'''
                You would be provided with article text which is text delimited by triple backticks..\n \n \
            ...
            if topic_name and topic_id:
                ...
                if len(client_list) > 0:
                    for row in client_list:
                        ...
                        retrieved_docs_list, similar_id_list = get_similar_docs_id(vectorstore, topic_name, sub_topic)
                        ...
                        if article_dict:
                            ...
                            if relevant_article_list:
                                ...
                                if article_for_client_list:
                                    ...
                                    langchain_best_match_found, langchain_best_article_id = get_best_id_source_chain(topic_id, topic_name, sub_topic, client_name, client_article_details_dict, emailids)
